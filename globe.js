...
async function initOperationsGlobe() {
  ...
  let viewLat = locations[0].lat;
  let viewLon = locations[0].lon;
  let targetLat = viewLat;
  let targetLon = viewLon;
  root.quaternion.copy(rotationFor(THREE, { lat: viewLat, lon: viewLon }, radius));
  ...
  canvas.addEventListener("pointermove", (event) => {
    ...
    if (dragging) {
      ...
      targetLon -= dx * 0.15;
      targetLat = clampLat(targetLat + dy * 0.12);
      viewLon = targetLon;
      viewLat = targetLat;
      lastX = event.clientX;
      lastY = event.clientY;
  ...
  function animate() {
    ...
    if (autoRotate) {
      targetLon += 0.13;
      viewLon = targetLon;
      viewLat += (targetLat - viewLat) * 0.08;
    } else if (!dragging) {
      viewLat += (targetLat - viewLat) * 0.08;
      viewLon += (targetLon - viewLon) * 0.08;
    }
    root.quaternion.copy(rotationFor(THREE, { lat: viewLat, lon: viewLon }, radius));
...
function rotationFor(THREE, location, radius) {
  const forward = latLonToVector(location.lat, location.lon, radius).normalize();
