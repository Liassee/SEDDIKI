...
async function initOperationsGlobe() {
  ...
  let lastX = 0;
  let lastY = 0;
  let targetQuat = rotationFor(THREE, locations[0], radius);
  ...
  const api = {
    focus(code) {
      ...
      if (!location) return;
      activeCode = code;
      targetQuat = rotationFor(THREE, location, radius);
      targetDistance = 2.9;
  ...
  stage.querySelector('[data-globe-action="focus-world"]')?.addEventListener("click", () => {
    targetQuat = new THREE.Quaternion().setFromEuler(new THREE.Euler(0.32, 0.05, 0, "YXZ"));
  ...
  canvas.addEventListener("pointermove", (event) => {
    ...
    if (dragging) {
      ...
      root.rotateOnWorldAxis(new THREE.Vector3(0, 1, 0), dx * 0.006);
      root.rotateOnWorldAxis(new THREE.Vector3(1, 0, 0), dy * 0.004);
      targetQuat.copy(root.quaternion);
      lastX = event.clientX;
      lastY = event.clientY;
      return;
    }
  ...
  function animate() {
    ...
    if (autoRotate) {
      root.rotateOnWorldAxis(new THREE.Vector3(0, 1, 0), 0.0022);
      targetQuat.copy(root.quaternion);
    } else if (!dragging) {
      root.quaternion.slerp(targetQuat, 0.06);
    }
  ...
  const pulse = 1 + Math.sin(
    performance.now() * 0.004 + mesh.userData.offset
  ) * 0.04;
...
function makeMarker(THREE, location, radius) {
  ...
  // Position géographique exacte sur la surface du globe
  const point = latLonToVector(location.lat, location.lon, radius + 0.025);

  group.position.copy(point);

  // Oriente le carré vers l'extérieur du globe
  ...
  // Petit carré
  const size = location.lead ? 0.032 : 0.020;
...
function rotationFor(THREE, location, radius) {
  const direction = latLonToVector(location.lat, location.lon, radius).normalize();
  return new THREE.Quaternion().setFromUnitVectors(direction, new THREE.Vector3(0, 0, 1));
}
